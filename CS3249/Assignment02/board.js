...
var Board = function(size) {
	...
	this.activeVictoryScreen = false;

	this.activateVictoryScreen = function(victorer){
		this.activeVictoryScreen = victorer;
	}

	this.disableVictoryScreen = function(){
		this.activeVictoryScreen = false;
	}
	...
	this.checkerObjects = {};
	this.checkerObjects.redPiece = {};
	this.checkerObjects.blackPiece = {};
	this.checkerObjects.redKing = {};
	this.checkerObjects.blackKing = {};

	this.checkerObjects.redPiece.img = new Image();
	this.checkerObjects.redPiece.img.src = "graphics/red-piece.png";
	this.checkerObjects.redPiece.loaded = false;

	this.checkerObjects.blackPiece.img = new Image();
	this.checkerObjects.blackPiece.img.src = "graphics/black-piece.png";
	this.checkerObjects.blackPiece.loaded = false;

	this.checkerObjects.redKing.img = new Image();
	this.checkerObjects.redKing.img.src = "graphics/red-king.png";
	this.checkerObjects.redKing.loaded = false;

	this.checkerObjects.blackKing.img = new Image();
	this.checkerObjects.blackKing.img.src = "graphics/black-king.png";
	this.checkerObjects.blackKing.loaded = false;

	this.checkerObjects.redPiece.img.onload = function() {
		board.checkerObjects.redPiece.loaded = true;
		board.draw();
	}

	this.checkerObjects.blackPiece.img.onload = function() {
		board.checkerObjects.redPiece.loaded = true;
		board.draw();
	}

	this.checkerObjects.redKing.img.onload = function() {
		board.checkerObjects.redKing.loaded = true;
		board.draw();
	}

	this.checkerObjects.blackKing.img.onload = function() {
		board.checkerObjects.blackKing.loaded = true;
		board.draw();
	}
	...
	this.checkIfVictory = function(whoseTurn){
		var checkers = this.getAllCheckers();

		var numBlack = 0;
		var numRed = 0;

		for(c in checkers){
			if(checkers[c].color == "black"){
				numBlack ++;
			} else {
				numRed ++;
			}
		...
		if(numBlack == 0){
			this.activateVictoryScreen("red");
		} else if (numRed == 0){
			this.activateVictoryScreen("black");
		}
	}
	...
	this.getRowColFromCoords = function(xCoord, yCoord) {
		var canvasWidth = document.getElementById('canvas').width;
		var squareSize = canvasWidth / this.boardSize;

		var col = Math.floor(xCoord / squareSize);
		var row = Math.floor(yCoord / squareSize);

		return [row, col];
	}

	this.draw = function(lastMove, dragInfo) {
		...
		var changeColor = function(currentColor, primaryColor, complementaryColor) {
			var color;
			if (currentColor == primaryColor.toLowerCase()) {
				color = complementaryColor;
			} else if (currentColor == complementaryColor.toLowerCase()) {
				color = primaryColor;
			} else {
				console.error("Color doesn't match primary or secondary!");
			}
			return color;
		}

		// Draw the squares on the board
		for (i = 0; i < (canvasWidth - 0.2 * squareSize); i += squareSize) {
			for (j = 0; j < (canvasHeight - 0.2 * squareSize); j += squareSize) {
				ctx.fillRect(i, j, squareSize, squareSize);
				ctx.fillStyle = changeColor(ctx.fillStyle, white, gray);
			}
			// Fix for even number of squares
		...
		if (lastMove) {
			ctx.beginPath();
			var arrowOriginX = (lastMove.details.fromCol + 1) * squareSize - (squareSize / 2);
			var arrowOriginY = (lastMove.details.fromRow + 1) * squareSize - (squareSize / 2);
			var arrowEndX = (lastMove.details.toCol + 1) * squareSize - (squareSize / 2);
			var arrowEndY = (lastMove.details.toRow + 1) * squareSize - (squareSize / 2);

			var gradient = ctx.createLinearGradient(arrowOriginX, arrowOriginY, arrowEndX, arrowEndY);
			
			gradient.addColorStop("0.1", 'rgba(255, 255, 255,0)');
			if(lastMove.details.checker.color == "black"){
				gradient.addColorStop("0.7", 'rgba(0, 0, 0,0.7)');
			} else {
				gradient.addColorStop("0.7", 'rgba(180, 0, 0,0.7)');
			}
		...
		ctx.restore();
		ctx.save();
		if (this.checkerObjects.redPiece.loaded && this.checkerObjects.redPiece.loaded) {
			for (c in checkers) {
				// User is moving piece 
				if(dragInfo && (checkers[c]==dragInfo.checker)){
					var offsetX = checkers[c].col * squareSize - dragInfo.originX;
					var offsetY = checkers[c].row * squareSize - dragInfo.originY;
					var posX = dragInfo.currentX + offsetX;
					var posY = dragInfo.currentY + offsetY;
					var originalCenterX = (checkers[c].col+1) * squareSize - (squareSize/2);
					var originalCenterY = (checkers[c].row+1) * squareSize - (squareSize/2);

					ctx.beginPath();
					var gradient = ctx.createLinearGradient(originalCenterX, originalCenterY, posX, posY);
					gradient.addColorStop("0.1", 'rgba(255, 255, 255,0)');
					gradient.addColorStop("0.5", 'rgba(240,240,20,1)');

					ctx.moveTo(originalCenterX, originalCenterY);
					ctx.lineTo(posX+squareSize/2, posY+squareSize/2);
					ctx.strokeStyle = gradient;
					ctx.lineWidth = 5;
					ctx.lineCap = "butt";
					ctx.stroke();
					...
					var posX = checkers[c].col * squareSize;
					var posY = checkers[c].row * squareSize;
				}

				if (checkers[c].isKing) {
					ctx.drawImage(this.checkerObjects[checkers[c].color + "King"].img, posX, posY, squareSize, squareSize);
				} else {
					ctx.drawImage(this.checkerObjects[checkers[c].color + "Piece"].img, posX, posY, squareSize, squareSize);
				}
			}
		}

		if(this.activeVictoryScreen){
			ctx.font = "45px Verdana";

			ctx.fillStyle = 'rgba(255,255,255,0.9)';
			ctx.fillRect(0, 0, 400, 400);

			if(this.activeVictoryScreen == "red"){
				ctx.fillStyle = 'rgb(180,0,0)';
				ctx.strokeStyle = 'rgb(180,0,0)';
			} else {
				ctx.fillStyle = this.activeVictoryScreen;	
				ctx.strokeStyle = this.activeVictoryScreen;
			}
			
			ctx.textAlign = "center";
			var victoryText = this.activeVictoryScreen.charAt(0).toUpperCase() + this.activeVictoryScreen.slice(1) + " Victory!"
			ctx.fillText(victoryText, canvasWidth/2, canvasHeight/2-25);
			...
			var infoText_line1 = "Press the New Game button to";
			var infoText_line2 = "the left if you want to play again!";
			ctx.fillText(infoText_line1, canvasWidth/2, canvasHeight/2+30);
			ctx.fillText(infoText_line2, canvasWidth/2, canvasHeight/2+45);
		}
	}
}